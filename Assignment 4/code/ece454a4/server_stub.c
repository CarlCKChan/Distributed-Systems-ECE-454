...
#ifdef _DEBUG_1_
    printf("returnResult(), about to exit");
    fflush(stdout); 
#endif
}
...
void launch_server() {
...
#ifdef _DEBUG_1_
	printf("launch_server(), in free(),before fname\n"); fflush(stdout);
	printf("launch_server(), in free(),fname is %p\n", fname); fflush(stdout);
#endif
	free(fname);
	if(a!=NULL)
	{
	...
	{
#ifdef _DEBUG_1_
	printf("launch_server(), in free(), a is NULL\n"); fflush(stdout);
#endif
	}
	//freeRet(ret);
	
#ifdef _DEBUG_1_
	printf("launch_server(), after free()\n"); fflush(stdout); 
#endif
