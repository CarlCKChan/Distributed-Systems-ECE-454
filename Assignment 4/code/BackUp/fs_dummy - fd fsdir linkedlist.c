...
struct fdIPMappingList* SearchFDIPMappingList(bool IsFD, int value)
{
...
FSDIR* fsOpenDir(const char *folderName)
{
	...
	int relPathIndex = searchCommonPath(mySrvPntr->localFolderName, folderName);
	char* relPathName = &folderName[relPathIndex];
	
	//struct stat sbuf;
	return_type ans;
	ans = make_remote_call(mySrvPntr->serverIP, mySrvPntr->port, "openDir", 1, strlen(relPathName)+1, (void*)relPathName);
...
int fsOpen(const char *fname, int mode)
{
	...
	int relPathIndex = searchCommonPath(mySrvPntr->localFolderName, fname);
	char* relPathName = &fname[relPathIndex];
	
	// Do RPC to server
	return_type ans;
	ans = make_remote_call(mySrvPntr->serverIP, mySrvPntr->port, "open", 4, 
...
int fsRemove(const char *name)
{
	...
	int relPathIndex = searchCommonPath(mySrvPntr->localFolderName, name);
	char* relPathName = &name[relPathIndex];
	
	// Do RPC to server
	return_type ans;
	ans = make_remote_call(mySrvPntr->serverIP, mySrvPntr->port, "remove", 3, 
							strlen(clientSideIp)+1, (void*)&clientSideIp,		// Client-side IP
							sizeof(clientSidePid), (void*)clientSidePid,
							strlen(relPathName)+1, (void*)relPathName
							);
